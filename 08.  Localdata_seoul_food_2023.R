...
foodshop <- foodshop %>%  filter(address != '서울특별시 동대문구 용두동 235-18' |  open_date != '20190708' |  close_date != '20210902') %>%  select(name,type,status,open_date,close_date,address)
...
#마지막으로 폐업년도만 분리해서 변수를 생성해 봅시다. 
foodshop$close_year<- substr(foodshop$close_date,1,4)

#다음으로 주소 변수에서 ’00구’만 분리해서 데이터를 만들어 봅시다.  주소값의 7~10구간의 데이터를 분리해 냅니다. 
...
#새로 생성한 인허가년도와 폐업년도가 문자형으로 생성되었습니다. 정수형으로 변경합니다. 
...
foodshop %>%  filter(!is.na(open_date)&status =="영업") %>% #결측치제거,  영업데이터 추출 
...
#4. 영업 중인 음식점의 업종별 비율 
foodshop %>%  
  filter(!is.na(open_date)&!is.na (type)&!is.na(district)) %>%  #결측치제외 filter(status=="영업") %>%  #영업만 추출 
  group_by(type) %>%  
  summarise(n=n()) %>%  #범주빈도계산 
  mutate(total=sum(n),pct=round(n/total*100,1)) %>%  #범주별비율계산 
...
#8. 폐업이 많았던 연도 
foodshop %>%  filter(!is.na(close_date)&!is.na (district))%>% #결측치제외 
  group_by(close_year) %>%  
  summarise(n=n()) %>%  #범주빈도계산 
  arrange(desc(n)) %>% 
  head(5)
...
close_trend <- foodshop %>%  
  filter(!is.na(open_date)&!is.na (district)) %>% #결측치제외 
  group_by(close_year) %>%  summarise(close_n=n()) #open_trend 구조 
str(close_trend) #연도별 개업 음식점수 막대그래프
